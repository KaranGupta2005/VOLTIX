...
const DecisionLogSchema = new mongoose.Schema({
  ...
  agent: {
    type: String,
    required: true,
    enum: ['MechanicAgent', 'TrafficAgent', 'LogisticsAgent', 'EnergyAgent', 'AuditorAgent']
  },
  action: {
    type: String,
    required: true
  ...
  triggerEvent: {
    type: String,
    required: true,
    enum: ['routine_monitoring', 'threshold_breach', 'user_request', 'system_alert', 'external_event', 'scheduled_task', 'emergency']
  },
  context: {
    inputData: {
...
DecisionLogSchema.methods.updateExecutionStatus = function(status, details = {}) {
  const updates = {
...
DecisionLogSchema.methods.linkRelatedDecision = function(decisionId, relationship) {
  return this.updateOne({
    $push: {
      'relatedDecisions': {
        decisionId,
...
DecisionLogSchema.methods.calculateEffectiveness = function() {
  ...
  const costEfficiency = this.impact.revenueImpact > 0 ? 
    Math.min(1, this.impact.revenueImpact / Math.abs(this.impact.costImpact)) : 0;
...
DecisionLogSchema.statics.getHighRiskDecisions = function(riskThreshold = 0.7) {
  return this.find({
...
DecisionLogSchema.statics.getPerformanceAnalytics = function(agent, timeRange = 168) {
  const since = new Date(Date.now() - timeRange * 60 * 60 * 1000);
