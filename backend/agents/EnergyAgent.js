...
class EnergyAgent extends BaseAgent {
  ...
  async detect(eventData) {
    try {
      ...
      if (currentPrice <= this.thresholds.price.low) {
        opportunities.push(
          `Low energy price: ₹${currentPrice}/kWh - buying opportunity`,
        );
      } else if (currentPrice >= this.thresholds.price.high) {
        opportunities.push(
          `High energy price: ₹${currentPrice}/kWh - selling opportunity`,
        );
      }
      ...
      if (demandLevel >= this.thresholds.demand.peak) {
        opportunities.push(
          `Peak demand: ${(demandLevel * 100).toFixed(
            1,
          )}% - price surge expected`,
        );
      } else if (demandLevel <= this.thresholds.demand.low) {
        opportunities.push(
          `Low demand: ${(demandLevel * 100).toFixed(
            1,
      ...
      if (storageLevel <= this.thresholds.storage.low / 100) {
        opportunities.push(
          `Low storage: ${(storageLevel * 100).toFixed(1)}% - charging needed`,
        );
      } else if (storageLevel >= this.thresholds.storage.full / 100) {
        opportunities.push(
          `High storage: ${(storageLevel * 100).toFixed(
            1,
          )}% - selling opportunity`,
        );
      }
      ...
      if (data.gridData) {
        ...
        if (
          grid.frequency < this.thresholds.gridStability.frequency.min ||
        ...
        if (
          grid.voltage < this.thresholds.gridStability.voltage.min ||
      ...
      if (data.renewableGeneration) {
        ...
        if (renewable.solar > renewable.capacity * 0.8) {
          opportunities.push(
            `High solar generation: ${renewable.solar}kW - excess available`,
          );
        }
        if (renewable.wind > renewable.capacity * 0.7) {
          opportunities.push(
            `High wind generation: ${renewable.wind}kW - excess available`,
          );
        }
      }
  ...
  async decide(eventData, context) {
    try {
      ...
      if (currentPrice <= this.thresholds.price.low && storageLevel < 0.6) {
        ...
        const buyAmount = this.calculateOptimalBuyAmount(
          currentPrice,
          storageLevel,
          demandLevel,
        );
      ...
      // High price + high storage = SELL opportunity
        ...
        currentPrice >= this.thresholds.price.high &&
        storageLevel > 0.7
      ) {
        action = "sell_energy";
        confidence = 0.8;
        ...
        const sellAmount = this.calculateOptimalSellAmount(
          currentPrice,
          storageLevel,
          demandLevel,
        );
      ...
      if (mlPrediction?.success) {
        ...
        if (
          mlData.predicted_price > currentPrice * 1.2 &&
          action === "monitor"
        ) {
          action = "buy_energy";
          riskScore = 0.3;
        } else if (
          mlData.predicted_price < currentPrice * 0.8 &&
          action === "monitor"
        ) {
          action = "sell_energy";
          riskScore = 0.3;
      ...
      return {
        ...
        riskScore,
        impact,
        reasoning: `Price: ₹${currentPrice}, Demand: ${(
          demandLevel * 100
        ).toFixed(1)}%, Storage: ${(storageLevel * 100).toFixed(1)}%`,
        mlPrediction: mlPrediction?.prediction,
        priority:
          currentPrice >= this.thresholds.price.critical ? "urgent" : "medium",
  ...
  async act(decision, eventData) {
    try {
      ...
      console.log(
        `[EnergyAgent] Executing ${action} on ${stationId} (Autonomy Level: ${autonomyLevel})`,
      );
  ...
  async executeEnergyPurchase(decision, eventData) {
    ...
    const marketPrice = decision.impact.costImpact
      ? Math.abs(decision.impact.costImpact) / buyAmount
      : 8;
    ...
    return {
      success: true,
      message: `Energy purchase executed: ${buyAmount}kWh at ₹${actualPrice.toFixed(
        2,
      )}/kWh`,
      impact: {
  ...
  async executeEnergySale(decision, eventData) {
    ...
    const marketPrice = decision.impact.revenueImpact
      ? decision.impact.revenueImpact / sellAmount
      : 12;
    ...
    return {
      success: true,
      message: `Energy sale executed: ${sellAmount}kWh at ₹${actualPrice.toFixed(
        2,
      )}/kWh (includes grid service premium)`,
      impact: {
  ...
  async executeSurgePricing(decision, eventData) {
    ...
    return {
      ...
      message: `Surge pricing activated: ${(
        (surgeMultiplier - 1) *
        100
      ).toFixed(0)}% increase with demand elasticity modeling`,
      impact: {
        surgeMultiplier,
  ...
  async executeGridSupport(decision, eventData) {
    const supportType =
      decision.impact.gridSupportType || "frequency_regulation";
  ...
  async executeRenewableOptimization(decision, eventData) {
    ...
    return {
      success: true,
      message: `Renewable optimization executed: ${(utilization * 100).toFixed(
        1,
  ...
  async verify(actionResult, eventData) {
    try {
      ...
      if (action.includes("purchase")) {
        ...
        reason = `Energy purchase completed with ${(
        ...
        reason = `Energy sale completed - market acceptance: ${(
          effectivenessScore * 100
        ).toFixed(1)}%`;
        ...
        reason = `Surge pricing implemented - user acceptance: ${(
          effectivenessScore * 100
        ).toFixed(1)}%`;
  ...
  assessGridStability(gridData) {
    ...
    const freqStable =
      gridData.frequency >= this.thresholds.gridStability.frequency.min &&
      gridData.frequency <= this.thresholds.gridStability.frequency.max;
    const voltStable =
      gridData.voltage >= this.thresholds.gridStability.voltage.min &&
  ...
  assessMarketConditions(data) {
    ...
    if (
      price <= this.thresholds.price.low &&
      demand <= this.thresholds.demand.low
    ) {
      return "buyer_market";
    } else if (
      price >= this.thresholds.price.high &&
      demand >= this.thresholds.demand.high
    ) {
