...
class TrafficAgent extends BaseAgent {
  ...
  async detect(eventData) {
    try {
      ...
      const utilizationRate = data.availableSlots ? 
        (data.capacity - data.availableSlots) / data.capacity : 0.5;
      ...
      if (eventData.type === 'user_event' && eventData.data.type === 'queue_join') {
  ...
  async decide(eventData, context) {
    try {
      ...
      if (queueLength >= this.thresholds.queueLength.critical || waitTime >= this.thresholds.waitTime.critical) {
        ...
        impact = {
          ...
          expectedRedirects: Math.floor(queueLength * incentiveAmount.acceptanceProbability)
        };
      }
      // Medium congestion - surge pricing
        ...
        const surgeMultiplier = this.calculateSurgeMultiplier(utilizationRate, context.isPeakHour);
        
        impact = {
          costImpact: 0,
          revenueImpact: this.estimateSurgeRevenue(queueLength, surgeMultiplier),
          successRate: 0.75,
          userSatisfaction: 0.65, // Users don't love surge pricing
          riskScore: 0.25,
          surgeMultiplier,
          expectedDemandReduction: this.calculateDemandReduction(surgeMultiplier)
        };
      }
      // Low utilization - attraction incentives
        ...
        const attractionIncentive = this.calculateAttractionIncentive(utilizationRate, context.isPeakHour);
  ...
  async act(decision, eventData) {
    try {
      ...
      console.log(`[TrafficAgent] Executing ${action} on ${stationId} (Autonomy Level: ${autonomyLevel})`);
      ...
      switch (action) {
        ...
        case 'preemptive_demand_shaping':
          ...
          result = await this.executePreemptiveDemandShaping(decision, eventData);
          break;
    ...
    try {
      ...
      if (action.includes('incentive')) {
        ...
        if (acceptanceRate < 0.4) {
          ...
          reason = `Incentive accepted by ${(acceptanceRate * 100).toFixed(1)}% of users`;
        }
      } else if (action.includes('pricing')) {
        // Dynamic pricing usually works but may reduce satisfaction
        effectivenessScore = 0.75;
        reason = 'Dynamic pricing reduced demand but may impact user satisfaction';
      }
  ...
  calculateAlgorithmicIncentive(queueLength, waitTime, nearbyStations, userProfile = {}) {
  ...
  calculateAcceptanceProbability(incentive, timeSaved, extraDistance, userProfile) {
    ...
    const finalProbability = Math.max(0.1, Math.min(0.9, 
      baseAcceptance + timeFactor - distancePenalty + urgencyBonus + pricesensitivity
    ));
  ...
  calculateAttractionIncentive(utilizationRate, isPeakHour) {
    ...
    const amount = Math.max(10, baseIncentive + utilizationFactor + peakPenalty);
  ...
  async executeIncentiveEngine(decision, eventData) {
    ...
    return {
      ...
      impact: {
        ...
        expectedAcceptance: Math.floor(notifications.length * incentiveDetails.acceptanceProbability),
  ...
  async executeSurgePricing(decision, eventData) {
    ...
    return {
      success: true,
      message: `Surge pricing activated: ${((surgeMultiplier - 1) * 100).toFixed(0)}% increase`,
      impact: {
        surgeMultiplier,
  ...
  async executePreemptiveDemandShaping(decision, eventData) {
    ...
    return {
      ...
      impact: {
        ...
        expectedEffectiveness: preemptiveAction.proactiveIncentives.expectedEffectiveness,
  ...
  isPeakHour() {
    ...
    return (currentTime >= 800 && currentTime <= 1000) || 
