...
class TrafficAgent extends BaseAgent {
  ...
  async identifyTargetUsers(eventData, context) {
    ...
    // Filter out users who already have an active coupon
    try {
      const candidateIds = candidates.map((c) => c.userId);
      const activeCoupons = await Notification.find({
        userId: { $in: activeIds },
        type: "incentive",
        "incentive.validUntil": { $gt: new Date() },
      }).select("userId");

      const usersWithCoupons = new Set(activeCoupons.map((n) => n.userId));

      return candidates.filter((user) => !usersWithCoupons.has(user.userId));
    } catch (error) {
      console.warn(
        "[TrafficAgent] Failed to check active coupons, filtering disabled:",
        error.message,
      );
      return candidates;
    }
