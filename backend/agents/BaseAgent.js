...
class BaseAgent {
  ...
  async processEvent(eventData, io) {
    ...
    try {
      console.log(
        `[${this.agentName}] Starting lifecycle for event: ${eventData.type}`,
      );
      this.lastActivity = new Date();
      ...
      if (!detectionResult.shouldProcess) {
        console.log(
          `[${this.agentName}] Event ignored: ${detectionResult.reason}`,
        );
        return {
          success: true,
          action: "ignored",
      ...
      console.log(
        `[${this.agentName}] Event detected: ${detectionResult.reason}`,
      );
      ...
      console.log(
        `[${this.agentName}] Decision made: ${decision.action} (confidence: ${decision.confidence})`,
      );
      ...
      if (
        this.config.requiresSupervisorApproval ||
        decision.riskScore > this.config.escalationThreshold
      ) {
        const approval = await this.requestSupervisorApproval(
          decision,
          decisionData,
        );
        if (!approval.approved) {
          console.log(
            `[${this.agentName}] Action blocked by supervisor: ${approval.reason}`,
          );
      ...
      console.log(
        `[${this.agentName}] Action executed: ${actionResult.message}`,
      );
      ...
      if (!verificationResult.success) {
        console.log(
          `[${this.agentName}] Verification failed: ${verificationResult.reason}`,
        );
        ...
        // Log failed decision
        try {
          const { default: decisionLogger } = await import(
          ...
          console.error(
            `[${this.agentName}] Failed decision logging error:`,
            logError,
          );
        }
      ...
      try {
        ...
        if (logResult.success) {
          console.log(
            `[${this.agentName}] Decision logged with explanation: ${logResult.decisionId}`,
          );
        } else {
          console.warn(
            `[${this.agentName}] Decision logging failed: ${logResult.error}`,
          );
        }
      } catch (logError) {
      ...
      try {
        ...
        console.error(
          `[${this.agentName}] Error decision logging failed:`,
          logError,
        );
      }
  ...
  async escalate(decision, actionResult, verificationResult, eventData, io) {
    try {
      console.log(
        `[${this.agentName}] Escalating failed action: ${decision.action}`,
      );

      // Determine escalation severity
      const severity = this.calculateEscalationSeverity(
        decision,
        verificationResult,
      );
      ...
      // Log escalation
      console.log(
        `[${this.agentName}] Escalation notification sent (severity: ${severity})`,
      );
    } catch (error) {
  ...
  async escalateSystemError(error, eventData, io) {
    try {
      ...
      console.error(
        `[${this.agentName}] System error escalation failed:`,
        escalationError,
      );
    }
  }
  ...
  async requestSupervisorApproval(decision, decisionData) {
    ...
    if (riskScore > 0.9 || confidence < 0.5) {
      return {
        approved: false,
        reason: `High risk (${riskScore.toFixed(
          2,
        )}) or low confidence (${confidence.toFixed(2)})`,
      };
    }
  ...
  getStats() {
    const successRate =
      this.processedEvents > 0
        ? ((this.successfulActions / this.processedEvents) * 100).toFixed(2)
        : 0;
