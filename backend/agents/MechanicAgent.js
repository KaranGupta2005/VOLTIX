...
class MechanicAgent extends BaseAgent {
    ...
    try {
      ...
      if (data.sensorData) {
        ...
        if (sensor.temperature > this.thresholds.temperature.max) {
          issues.push(`OVERHEATING: ${sensor.temperature}°C (Critical: ${this.thresholds.temperature.critical}°C)`);
        }
        
        // Voltage stability analysis
        if (sensor.voltage < this.thresholds.voltage.min || sensor.voltage > this.thresholds.voltage.max) {
          const issue = sensor.voltage < this.thresholds.voltage.min ? 'POWER_DROP' : 'POWER_SURGE';
          issues.push(`${issue}: ${sensor.voltage}V (Range: ${this.thresholds.voltage.min}-${this.thresholds.voltage.max}V)`);
        }
        
        // Current anomaly detection
        if (sensor.current > this.thresholds.current.max) {
          issues.push(`OVERCURRENT: ${sensor.current}A (Max: ${this.thresholds.current.max}A)`);
        }
        
        // Vibration analysis for mechanical issues
        if (sensor.vibration > this.thresholds.vibration.max) {
          issues.push(`MECHANICAL_STRESS: ${sensor.vibration}G (Max: ${this.thresholds.vibration.max}G)`);
        }
      }
      ...
      if (data.performance) {
        if (data.performance.errorRate > this.thresholds.errorRate.max) {
          issues.push(`HIGH_ERROR_RATE: ${(data.performance.errorRate * 100).toFixed(1)}%`);
        }
  ...
  async act(decision, eventData) {
    try {
      ...
      console.log(`[MechanicAgent] Executing ${action} on ${stationId} (Autonomy Level: ${autonomyLevel})`);
      ...
      if (result.selfHealed) {
        result.verificationSteps = await this.performSelfHealingVerification(stationId, action);
      }
  ...
  performRootCauseAnalysis(issues, data) {
    ...
    issues.forEach(issue => {
      ...
      } else if (issue.includes('POWER_DROP') || issue.includes('POWER_SURGE')) {
  ...
  createRecoveryPlan(rootCause, severity) {
    const plans = {
      'OVERHEATING': {
        ...
        steps: ['pause_charging', 'activate_cooling', 'monitor_temperature', 'gradual_restart'],
        estimatedTime: '5-10 minutes',
        successRate: 0.92
      },
      'POWER_DROP': {
        strategy: 'power_stabilization',
        steps: ['isolate_faulty_slot', 'switch_to_backup_grid', 'voltage_regulation', 'restart_charging'],
        estimatedTime: '2-5 minutes',
        successRate: 0.88
      },
      'NETWORK_FAIL': {
        strategy: 'connectivity_restoration',
        steps: ['reset_network_interface', 'reconnect_to_network', 'verify_connectivity', 'resume_operations'],
        estimatedTime: '1-3 minutes',
        successRate: 0.85
      },
      'PROTOCOL_TIMEOUT': {
        strategy: 'protocol_recovery',
        steps: ['clear_communication_buffer', 'firmware_rollback', 'restart_communication', 'verify_protocol'],
        estimatedTime: '3-7 minutes',
        successRate: 0.87
      },
      'MECHANICAL_STRESS': {
        strategy: 'mechanical_stabilization',
        steps: ['reduce_operational_load', 'activate_dampening', 'schedule_inspection', 'monitor_vibration'],
        estimatedTime: '10-30 minutes',
        successRate: 0.75
      },
      'HIGH_ERROR_RATE': {
        strategy: 'system_recovery',
        steps: ['run_system_diagnostics', 'clear_error_logs', 'restart_services', 'verify_operations'],
        estimatedTime: '5-15 minutes',
        successRate: 0.83
      }
    };
    
    return plans[rootCause.primaryCause] || {
      strategy: 'emergency_isolation',
      steps: ['immediate_shutdown', 'isolate_station', 'alert_human_operator', 'await_manual_intervention'],
  ...
  calculateFailureRisk(data) {
    ...
    // Temperature risk
    if (data.sensorData?.temperature > this.thresholds.temperature.critical) riskScore += 0.4;
    else if (data.sensorData?.temperature > this.thresholds.temperature.max) riskScore += 0.2;
    
    // Voltage risk
    if (data.sensorData?.voltage < this.thresholds.voltage.critical || 
        data.sensorData?.voltage > this.thresholds.voltage.critical) riskScore += 0.3;
    
    // Performance risk
    if (data.performance?.errorRate > this.thresholds.errorRate.critical) riskScore += 0.2;
  ...
  async executeCoolingProtocol(stationId) {
    ...
    return {
      success: true,
      message: 'Cooling protocol executed successfully - temperature normalized',
      impact: {
  ...
  async executePreventiveMaintenance(stationId) {
    ...
    return {
      success: true,
      message: 'Preventive maintenance scheduled - proactive failure prevention',
      impact: {
  ...
  calculateSeverity(data) {
    ...
    if (data.sensorData) {
      ...
      if (sensor.temperature > this.thresholds.temperature.critical) severityScore += 3;
      else if (sensor.temperature > this.thresholds.temperature.max) severityScore += 1;
      
      if (sensor.voltage < this.thresholds.voltage.critical || sensor.voltage > this.thresholds.voltage.critical) severityScore += 3;
      else if (sensor.voltage < this.thresholds.voltage.min || sensor.voltage > this.thresholds.voltage.max) severityScore += 1;
      
      if (sensor.vibration > this.thresholds.vibration.critical) severityScore += 2;
      else if (sensor.vibration > this.thresholds.vibration.max) severityScore += 1;
    }
    
    if (data.performance?.errorRate > this.thresholds.errorRate.critical) severityScore += 2;
  ...
  identifyAffectedSystems(data) {
    ...
    if (data.sensorData?.voltage < this.thresholds.voltage.min || data.sensorData?.voltage > this.thresholds.voltage.max) {
