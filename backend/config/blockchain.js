...
class BlockchainConfig {
  constructor() {
    ...
    if (process.env.AUDIT_CONTRACT_ADDRESS) {
      this.AUDIT_CONTRACT_ADDRESS = process.env.AUDIT_CONTRACT_ADDRESS;
      console.log(
        `✅ Using AUDIT_CONTRACT_ADDRESS from environment: ${this.AUDIT_CONTRACT_ADDRESS}`,
      );
    } else {
      // 2. Check File (deployment.json)
      try {
        ...
        if (fs.existsSync(deploymentPath)) {
          ...
          // Look for 'contractAddress', 'address', or 'auditContract'
          const address =
            data.contractAddress || data.address || data.auditContract;

          if (address) {
            this.AUDIT_CONTRACT_ADDRESS = address;
            console.log(
              `📂 Loaded AUDIT_CONTRACT_ADDRESS from deployment.json: ${this.AUDIT_CONTRACT_ADDRESS}`,
            );
          }
        }
      } catch (error) {
        console.warn(`⚠️ Failed to read deployment.json: ${error.message}`);
      }
    }

    // 3. Fallback
    if (!this.AUDIT_CONTRACT_ADDRESS) {
      this.AUDIT_CONTRACT_ADDRESS =
        "0xcd3b766ccdd6ae721141f452c550ca635964ce71";
      console.log(
        `⚠️ Using Fallback Default AUDIT_CONTRACT_ADDRESS: ${this.AUDIT_CONTRACT_ADDRESS}`,
      );
    }
    this.PRIVATE_KEY =
  ...
  async initialize() {
    try {
      ...
      let totalLogs = 0;
      try {
        totalLogs = await this.auditContract.totalLogs();
        console.log(`Total audit logs on chain: ${totalLogs}`);
      } catch (err) {
        if (err.code === "BAD_DATA" && err.value === "0x") {
          console.error(
            `❌ CONTRACT MISSING: No contract found at ${this.AUDIT_CONTRACT_ADDRESS}.`,
          );
          console.error(
            `💡 HINT: You likely restarted 'npx hardhat node'. This resets the blockchain.`,
          );
          console.error(
            `👉 ACTION: Run 'node scripts/deploy-simple.js' in the backend folder to redeploy.`,
          );
          throw new Error(
            `Contract missing at ${this.AUDIT_CONTRACT_ADDRESS}. Please redeploy.`,
          );
        }
        throw err;
      }
