...
class DecisionLogger {
  ...
  async logDecision(decisionData, io = null) {
    try {
      console.log(`[DecisionLogger] Logging decision: ${decisionData.agent} - ${decisionData.action}`);
      ...
      try {
        const explanationResult = await explainabilityService.generateExplanation({
          ...
          prediction: decisionData.predictionInsights || decisionData.mlMetrics,
        ...
        if (explanationResult.success) {
          ...
          console.log(`[DecisionLogger] Generated explanation for ${decisionData.agent}`);
        } else {
          explanationError = explanationResult.error;
          explanation = explanationResult.explanation; // Fallback explanation
          console.warn(`[DecisionLogger] Explanation generation failed, using fallback`);
        }
      ...
      try {
        ...
        if (auditResult.success) {
          ...
          console.log(`[DecisionLogger] Blockchain audit completed: ${transactionHash}`);
        } else {
          console.warn(`[DecisionLogger] Blockchain audit failed: ${auditResult.error}`);
        }
      } catch (blockchainError) {
        console.warn(`[DecisionLogger] Blockchain service unavailable:`, blockchainError.message);
      }
      ...
      console.log(`[DecisionLogger] Decision logged successfully: ${savedDecision.decisionId}`);
    ...
    try {
      ...
      return {
        ...
        stats: {
          ...
          explanationRate: Math.round((result.explanationsGenerated / result.totalDecisions) * 100),
          blockchainRate: Math.round((result.blockchainAudited / result.totalDecisions) * 100),
          timeRange
        }
      };
  ...
  async regenerateExplanation(decisionId) {
    try {
      ...
      decision.explanationError = explanationResult.success ? null : explanationResult.error;
      ...
      console.error(`[DecisionLogger] Failed to regenerate explanation:`, error);
      return {
        success: false,
        error: error.message
      };
