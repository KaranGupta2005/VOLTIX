    ...
    try {
      ...
      const mechanicEvent = {
        ...
        data: {
          ...
          errorCodes: station.errors.recentErrors.filter(e => !e.resolved).map(e => e.code),
      ...
      if (mechanicDetection.shouldProcess) {
        const mechanicDecision = await mechanicAgent.decide(mechanicEvent, mechanicDetection.context);
        const mechanicAction = await mechanicAgent.act(mechanicDecision, mechanicEvent);
        ...
        if (mechanicDecision.riskScore > 0.7) {
          await this.logToBlockchain('mechanic', station.id, mechanicDecision, mechanicAction);
        }
      ...
      if (trafficDetection.shouldProcess) {
        const trafficDecision = await trafficAgent.decide(trafficEvent, trafficDetection.context);
        const trafficAction = await trafficAgent.act(trafficDecision, trafficEvent);
        ...
        // Log to blockchain
        await this.logToBlockchain('traffic', station.id, trafficDecision, trafficAction);
      ...
      if (logisticsDetection.shouldProcess) {
        const logisticsDecision = await logisticsAgent.decide(logisticsEvent, logisticsDetection.context);
        const logisticsAction = await logisticsAgent.act(logisticsDecision, logisticsEvent);
        ...
        // Log to blockchain
        await this.logToBlockchain('logistics', station.id, logisticsDecision, logisticsAction);
      ...
      if (energyDetection.shouldProcess) {
        const energyDecision = await energyAgent.decide(energyEvent, energyDetection.context);
        ...
        // Log to blockchain
        await this.logToBlockchain('energy', station.id, energyDecision, energyAction);
      }
      ...
      if (Object.values(decisions).some(d => d !== null)) {
        ...
        if (auditDetection.shouldProcess) {
          const auditDecision = await auditorAgent.decide(auditEvent, auditDetection.context);
          ...
          await this.logToBlockchain('audit', station.id, auditDecision, auditAction);
        }
      }
  ...
  async logToBlockchain(agentType, stationId, decision, action) {
    try {
      ...
      if (logResult.success) {
        console.log(`✅ [${agentType.toUpperCase()}] Decision logged: ${logResult.decisionId}`);
    ...
    if (stationId) {
      ...
      if (station) {
        socket.emit('station-metrics-update', {
          ...
          inventory: station.inventory,
          errors: station.errors,
          agentDecisions: stationSocketHandler.agentDecisions.get(station.id) || {},
          timestamp: new Date().toISOString()
        });
      }
    }
