...
class MLClient {
  constructor() {
    ...
    this.client.interceptors.request.use(
      (config) => {
        console.log(
          `[ML Client] ${config.method?.toUpperCase()} ${config.url}`,
        );
        return config;
      },
      (error) => {
  ...
  async predictTrafficDemand(stationData, forecastHours = 4) {
    try {
      ...
      throw new Error(
        `Traffic demand prediction failed: ${
          error.response?.data?.detail || error.message
        }`,
      );
    }
  ...
  async predictEnergyPrices(marketData, forecastHours = 24) {
    try {
      ...
      console.error("ML Service Error (predictEnergyPrices):", error.message);
      // Fallback for demo/dev if ML service is down
      return {
        predicted_prices: Array(forecastHours)
          .fill(0)
          .map(() => 8 + Math.random() * 4),
        confidence: 0.7,
        forecast_hours: forecastHours,
  ...
  async batchAnalyzeDecisions(decisions) {
    try {
      ...
      throw new Error(
        `Batch decision analysis failed: ${
          error.response?.data?.detail || error.message
        }`,
      );
    }
  ...
  async optimizeMultiStopRoute(startLocation, stops, endLocation = null) {
    try {
      ...
      throw new Error(
        `Multi-stop route optimization failed: ${
          error.response?.data?.detail || error.message
        }`,
      );
    }
  ...
  async retryRequest(requestFn, attempts = this.retryAttempts) {
    for (let i = 0; i < attempts; i++) {
      try {
        ...
        console.log(
          `[ML Client] Retry attempt ${i + 1}/${attempts} after ${delay}ms`,
        );
