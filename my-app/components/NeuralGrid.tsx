        ...
        <section className="relative min-h-[120vh] bg-white overflow-hidden flex flex-col items-center justify-center py-24 border-t border-neutral-200">
            ...
            <div className="relative z-10 w-full max-w-7xl px-4 flex flex-col items-center">
                ...
                <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-0 relative min-h-[600px]">
                    ...
                    {/* ZONE 2: FUSION CORE (THE BRAIN) - COLS 4-8 */}
                    <div className="lg:col-span-5 flex items-center justify-center relative z-20">
                        ...
                        <div className="relative w-full h-[400px] flex items-center justify-center">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                ...
                                <div className="absolute top-[15%] text-[10px] font-mono text-emerald-500/50">State Cache (Redis)</div>
                            </div>
                            ...
                            <div className="w-40 h-40 bg-black/80 backdrop-blur-xl border border-emerald-500/30 rounded-full flex flex-col items-center justify-center relative z-10 shadow-[0_0_50px_rgba(16,185,129,0.1)]">
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="text-[10px] text-emerald-400 font-mono">LangGraph Core</div>
                                </div>
                                {/* Pulse Effect */}
                    ...
                    {/* ZONE 3: SUPERVISOR BRANCHING (EXECUTION) - COLS 9-12 */}
                    <div className="lg:col-span-4 flex flex-col justify-center relative z-20 pl-8">
                        ...
                        <div className="flex flex-col gap-6 relative">
                            ...
                            <AgentBranch icon={<Wrench />} label="Mechanic" color="text-red-400" border="border-red-500/30" />
                            <AgentBranch icon={<TrafficCone />} label="Traffic" color="text-purple-400" border="border-purple-500/30" />
                            <AgentBranch icon={<Truck />} label="Logistics" color="text-blue-400" border="border-blue-500/30" />
                            <AgentBranch icon={<Zap />} label="Energy" color="text-amber-400" border="border-amber-500/30" />
                            <AgentBranch icon={<ShieldCheck />} label="Auditor" color="text-neutral-400" border="border-neutral-500/30" />
                        </div>
                    </div>
                    ...
                    <svg className="absolute inset-0 w-full h-full pointer-events-none z-0 overflow-visible">
                        ...
                        <path d="M 15% 25% C 25% 25%, 35% 50%, 50% 50%" stroke="#3b82f6" strokeWidth="1" fill="none" className="opacity-30" />

                        {/* Path 2: Request (Purple) */}
                        <path d="M 15% 50% L 50% 50%" stroke="#a855f7" strokeWidth="1" fill="none" className="opacity-30" />

                        {/* Path 3: Forecast (Orange) */}
                        <path d="M 15% 75% C 25% 75%, 35% 50%, 50% 50%" stroke="#f97316" strokeWidth="1" fill="none" className="opacity-30" />


                        {/* --- CONNECTION C: FUSION -> EXECUTION --- */}
                        {/* Trunk: Core -> Router */}
                        <path d="M 50% 50% C 60% 50%, 60% 20%, 75% 20%" stroke="#10b981" strokeWidth="1" fill="none" className="opacity-40" />

                        {/* Branches: Router -> Agents */}
                        {/* To Mechanic */}
                        <path d="M 75% 20% L 90% 25%" stroke="#ef4444" strokeWidth="1" fill="none" className="opacity-30" />

                        {/* To Traffic */}
                        <path d="M 75% 20% C 80% 20%, 80% 35%, 90% 35%" stroke="#a855f7" strokeWidth="1" fill="none" className="opacity-30" />

                        {/* To Logistics */}
                        <path d="M 75% 20% C 80% 20%, 80% 50%, 90% 50%" stroke="#3b82f6" strokeWidth="1" fill="none" className="opacity-30" />

                        {/* To Energy */}
                        <path d="M 75% 20% C 80% 20%, 80% 65%, 90% 65%" stroke="#f59e0b" strokeWidth="1" fill="none" className="opacity-30" />

                        {/* To Auditor */}
                        <path d="M 75% 20% C 80% 20%, 80% 80%, 90% 80%" stroke="#737373" strokeWidth="1" fill="none" className="opacity-30" />
                        ...
                        <motion.circle
                            ...
                            style={{ offsetPath: 'path("M 15% 25% C 25% 25%, 35% 50%, 50% 50% C 60% 50%, 60% 20%, 75% 20% L 90% 25%")' }}
                        ...
                        <motion.circle
                            ...
                            style={{ offsetPath: 'path("M 15% 50% L 50% 50% C 60% 50%, 60% 20%, 75% 20% C 80% 20%, 80% 35%, 90% 35%")' }}
                            animate={{ offsetDistance: ["0%", "100%"] }}
                            transition={{ duration: 3, ease: "easeInOut", repeat: Infinity, repeatDelay: 0.5, delay: 1 }} // Offset start
                        />
                        ...
                        <motion.circle
                            ...
                            style={{ offsetPath: 'path("M 50% 50% C 60% 50%, 60% 20%, 75% 20% C 80% 20%, 80% 80%, 90% 80%")' }}
                            animate={{ offsetDistance: ["0%", "100%"] }}
                            transition={{ duration: 5, ease: "linear", repeat: Infinity, repeatDelay: 0, delay: 2 }}
                        />
...
function ModelStack({ title, color, layers, icon }: { title: string, color: 'blue' | 'purple' | 'amber' | 'gray', layers: string[], icon: any }) {
    ...
    return (
        <div className="w-full max-w-[280px] group">
            <div className="flex items-center gap-2 mb-3 px-1">
                {icon}
                <span className={cn("text-xs font-bold uppercase tracking-wider", colors[color].split(" ")[0])}>{title}</span>
            </div>
                ...
                {layers.map((layer, i) => (
                    <div key={i} className={cn(
                        ...
                        <div className="flex justify-between items-center z-10 relative">
                            ...
                            <div className={cn("w-1.5 h-1.5 rounded-full animate-pulse", dotColors[color])} />
                        </div>
...
function AgentBranch({ icon, label, color, border }: any) {
    return (
        <div className="flex items-center gap-4 relative group">
            <div className="w-6 h-px bg-neutral-800 group-hover:bg-neutral-600 transition-colors" /> {/* Horizontal connector */}
            ...
            <div className={cn(
                ...
                <div className={cn("text-neutral-400 group-hover:text-white transition-colors")}>
                    {icon}
                </div>
                <span className={cn("text-xs font-mono font-bold transition-colors", color)}>
                    {label}
                </span>
            </div>
        </div>
...
function Particle({ path, color, delay, duration }: { path: string, color: string, delay: number, duration: number }) {
    return (
        <motion.path
            d={path}
            stroke={color}
